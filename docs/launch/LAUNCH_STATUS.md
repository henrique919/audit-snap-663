...
| LP-25 | Fix ActionSheet backdrop nested buttons | P2 | **READY — fix queued** | Cursor | — | Cursor flagged during LP-23: when the ActionSheet opens, its backdrop button nests the option buttons (same RN-web `<button>`-in-`<button>` class as LP-23). Low blast radius (only while a sheet is open), but same invalid-HTML/SR issue. Make the backdrop a non-button pressable (e.g. Pressable without role=button, or a View with an onPress overlay that isn't announced as a button) with options as siblings. | Cursor fix |
